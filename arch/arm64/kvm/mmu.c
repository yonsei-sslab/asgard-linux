			  ...
			  struct kvm_memory_slot *memslot, unsigned long hva)
{
	...
	if (ret == -EHWPOISON) {
		...
		VM_BUG_ON(write_fault);

		/* 
		 * VFIO Platform driver mmaps MMIO regions with remap_pfn_range(), which
		 * will result in "special" PTEs with VM_IO and VM_PFNMAP flags set.
		 * pin_user_pages() do not like these special PTEs at all and will just
		 * fail (special PTEs aren't backed by struct page). We try again with
		 * __gfn_to_pfn_memslot(), which is proven to work well with this type
		 * of PTE in user_mem_abort().
		 */
		pfn = __gfn_to_pfn_memslot(memslot, fault_ipa >> PAGE_SHIFT, false,
					   NULL, write_fault, NULL);

		is_fault_handled = true;
		spin_lock(&kvm->mmu_lock);
		goto fault_handled;
