...
void __vgic_v3_restore_state(struct vgic_v3_cpu_if *cpu_if, bool is_device_attached)
{
	u64 used_lrs = cpu_if->used_lrs;
	int i;
	...
	if (used_lrs || cpu_if->its_vpe.its_vm) {
		...
		/* Enable all vIRQs when the VFIO device is attached */
		if (is_device_attached) {
			...
			for (i = 0; i < used_lrs; i++) {
				/* Disable RKNPU-sourced vIRQs when the device is not attached */
				if (((cpu_if->vgic_lr[i] & ICH_LR_VIRTUAL_ID_MASK) < 39) ||
					((cpu_if->vgic_lr[i] & ICH_LR_VIRTUAL_ID_MASK) > 41)) {
					__gic_v3_set_lr(cpu_if->vgic_lr[i], i);
				}
			}
		}
