...
static int create_hyp_debug_uart_mapping(void)
{
	...
	arm64_kvm_hyp_debug_uart_addr = (void *)__pkvm_create_private_mapping(base, PAGE_SIZE, PAGE_HYP_DEVICE);
	/* No need to encode error pointer into a pointer. */
...
void __noreturn __pkvm_init_finalise(void)
{
	...
	/* Create EL2 mapping of Rockchip clock-controller registers. */
	/* TODO: Do not hardcode Rockchip clock-controller registers. */
	rockchip_cru_base = (void *)__pkvm_create_private_mapping(0xfd7c0000, 0x1000,
							PAGE_HYP_DEVICE);
	if (IS_ERR(rockchip_cru_base))
		goto out;
