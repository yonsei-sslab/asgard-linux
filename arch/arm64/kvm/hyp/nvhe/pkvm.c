...
bool kvm_handle_pvm_hvc64(struct kvm_vcpu *vcpu, u64 *exit_code)
{
	...
	int ret;
	unsigned long reset_start_time, reset_end_time;
	...
	case ARM_SMCCC_VENDOR_HYP_KVM_ATTACH_DEVICE_FUNC_ID:
		asm volatile("isb; mrs %0, cntpct_el0" : "=r"(vcpu->hypcall_handle_start_time));
		...
		val[0] = SMCCC_RET_SUCCESS;
		asm volatile("isb; mrs %0, cntpct_el0" : "=r"(vcpu->hypcall_handle_end_time));
		break;
	case ARM_SMCCC_VENDOR_HYP_KVM_DETACH_DEVICE_FUNC_ID:
		...
		asm volatile("isb; mrs %0, cntpct_el0" : "=r"(reset_start_time));
		/* Temporarily disable RKNPU reset */
		// pkvm_reset_rknpu();
		asm volatile("isb; mrs %0, cntpct_el0" : "=r"(reset_end_time));
		vcpu->reset_time = reset_end_time - reset_start_time;
	...
	case ARM_SMCCC_VENDOR_HYP_KVM_GET_HYP_PERF_NUMS_FUNC_ID:
		val[0] = SMCCC_RET_SUCCESS;
		val[1] = vcpu->hypcall_handle_start_time;
		val[2] = vcpu->hypcall_handle_end_time;
		val[3] = vcpu->reset_time;
		break;
