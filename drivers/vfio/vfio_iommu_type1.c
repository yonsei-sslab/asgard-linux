...
static int vfio_iommu_type1_test_map_dma(struct vfio_iommu *iommu,
				    unsigned long arg)
{
	struct vfio_iommu_type1_test_dma_map map;
	struct vfio_domain *d;
	unsigned long minsz;
	int ret;

	minsz = offsetofend(struct vfio_iommu_type1_test_dma_map, size);
	...
	/* Multi-IOMMU is not supported. */
	d = list_first_entry(&iommu->domain_list, struct vfio_domain, next);

	while (map.size) {
		ret = iommu_map(d->domain, map.iova, (phys_addr_t)map.pfn << PAGE_SHIFT,
				map.npage << PAGE_SHIFT, IOMMU_READ | IOMMU_WRITE);
		if (ret)
			return ret;

		map.pfn += map.npage;
		map.iova += map.npage << PAGE_SHIFT;
		map.size -= map.npage << PAGE_SHIFT;
	}

	return 0;
}

static int vfio_iommu_type1_test_unmap_dma(struct vfio_iommu *iommu,
				    unsigned long arg)
{
	struct vfio_iommu_type1_test_dma_unmap unmap;
	struct vfio_domain *d;
	unsigned long minsz;
	int ret;

	minsz = offsetofend(struct vfio_iommu_type1_test_dma_unmap, size);
	...
	while (unmap.size) {
		ret = iommu_unmap(d->domain, unmap.iova, unmap.npage << PAGE_SHIFT);
		if (ret != unmap.npage << PAGE_SHIFT)
			return ret;

		unmap.iova += unmap.npage << PAGE_SHIFT;
		unmap.size -= unmap.npage << PAGE_SHIFT;
	}

	return 0;
}

static int vfio_iommu_type1_test_flush_iotlb_all(struct vfio_iommu *iommu)
{
	struct vfio_domain *d;
	...
	iommu_flush_iotlb_all(d->domain);
				   ...
				   unsigned int cmd, unsigned long arg)
{
	...
	case VFIO_IOMMU_TEST_MAP_DMA:
		return vfio_iommu_type1_test_map_dma(iommu, arg);
	case VFIO_IOMMU_TEST_UNMAP_DMA:
		return vfio_iommu_type1_test_unmap_dma(iommu, arg);
	case VFIO_IOMMU_TEST_FLUSH_IOTLB_ALL:
		return vfio_iommu_type1_test_flush_iotlb_all(iommu);
	default:
		return -ENOTTY;
	}
}
