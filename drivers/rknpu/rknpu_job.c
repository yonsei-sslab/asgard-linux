...
static struct rknpu_drv_comp_perf_eval drv_comp_perf_eval;
static int rknpu_job_submit_cnt = 0;
			...
			struct rknpu_submit *args)
{
	struct rknpu_job *job = NULL;
	int ret = -EINVAL;
	unsigned long long cnt_1, cnt_2;

	/* Get timer counter value */
	asm volatile("isb; mrs %0, cntpct_el0" : "=r"(cnt_1));
	...
	/* Get timer counter value */
	asm volatile("isb; mrs %0, cntpct_el0" : "=r"(cnt_2));

	/* Add the difference to cnt_total */
	if (drv_comp_perf_eval.cnt_limit > rknpu_job_submit_cnt) {
		drv_comp_perf_eval.cnt_total += cnt_2 - cnt_1;
		rknpu_job_submit_cnt++;
	}
...
int rknpu_get_drv_comp_perf_eval_ioctl(struct drm_device *dev, void *data,
			struct drm_file *file_priv)
{
	struct rknpu_drv_comp_perf_eval *args = data;

	args->cnt_total = drv_comp_perf_eval.cnt_total;

	return 0;
}

int rknpu_clear_drv_comp_perf_eval_ioctl(struct drm_device *dev, void *data,
			struct drm_file *file_priv)
{
	rknpu_job_submit_cnt = 0;
	drv_comp_perf_eval.cnt_total = 0;

	return 0;
}

int rknpu_set_drv_comp_perf_eval_ioctl(struct drm_device *dev, void *data,
			struct drm_file *file_priv)
{
	struct rknpu_drv_comp_perf_eval *args = data;

	drv_comp_perf_eval.cnt_limit = args->cnt_limit;
